...
module bed_mount(){ 
	...
	linear_extrude(height = 10, convexity = 5) difference() {
		union() {
			...
			translate([0, -rod_size / 2 - 4-12.5, 0]) square([rod_size / 2 + 8, max(rod_size + 25.5, rod_size / 2 + 12 )]);
		}
	...
	translate([rod_size / 2 + 1.5, -rod_size / 2 - 19.5, 5]) rotate([-90, 0, 0]) {
		cylinder(r = m4_size * da6, h = max(rod_size + 24, rod_size / 2 + 7 + bed_mount_height+20, $fn = 6));
		cylinder(r = m4_nut_size * da6, h = 16, $fn = 6);
	}
}
